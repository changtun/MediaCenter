/*
 * File Name:  CustomFlingGalleryView.java
 ...
 */
public class CustomFlingGalleryView extends ViewGroup
{
    ...
    public CustomFlingGalleryView(Context context)
    {
        super(context);
        init();
        mCurrentScreen = 0;
    ...
    public CustomFlingGalleryView(Context context, AttributeSet attrs)
    {
        this(context, attrs, 0);
    }
    
    public CustomFlingGalleryView(Context context, AttributeSet attrs, int defStyle)
    {
    ...
    /*
     * down事件首先会传递到onInterceptTouchEvent()方法 如果该ViewGroup的onInterceptTouchEvent()在接收到down事件处理完成之后return false，那么后续的move,
     * up等事件将继续会先传递给该ViewGroup，之后才和down事件一样传递给最终的目标view的onTouchEvent()处理。
     * 如果该ViewGroup的onInterceptTouchEvent()在接收到down事件处理完成之后return true，那么后续的move,
     * up等事件将不再传递给onInterceptTouchEvent()，而是和down事件一样传递给该ViewGroup的onTouchEvent ()处理，注意，目标view将接收不到任何事件。
     * 如果最终需要处理事件的view的onTouchEvent()返回了false，那么该事件将被传递至其上一层次的view的onTouchEvent ()处理。 如果最终需要处理事件的view
